...
function run_swift {
  ...
  xcodebuild test -project AlibabaCloudRoaUtils.xcodeproj -scheme "AlibabaCloudRoaUtils-Package" -sdk "macosx" -destination "platform=OS X,arch=x86_64" -configuration Debug ONLY_ACTIVE_ARCH=NO test
  cd ../
...
function run_python {
  ...
  coverage run --source="./alibabacloud_roa_util" ./tests/run_test.py
  cd ../
